...
txt_monitor_label = Text(text="MONITOR DE SISTEMA OPERACIONAL:", position=(-0.82, 0.28), scale=1.0, color=color.light_gray)
status_panel = Entity(parent=camera.ui, model='quad', scale=(0.36, 0.05), position=(-0.65, 0.22), color=color.cyan)
status_text = Text(text="HOMEOSTASE DINÂMICA ESTÁVEL", position=(-0.81, 0.23), scale=1.0, color=color.black)

txt_coherence = Text(text="Phase Coherence: 100.00%", position=(-0.82, 0.14), scale=1.0, color=color.white)
txt_entropy = Text(text="Entropia de Shannon: 0.0000 bits", position=(-0.82, 0.10), scale=1.0, color=color.white)
txt_bohm = Text(text="Pressão de Bohm (Q): 0.00 eV", position=(-0.82, 0.06), scale=1.0, color=color.white)
txt_gravity = Text(text="Delta G (Δg): 0.0000 m/s²", position=(-0.82, 0.02), scale=1.0, color=color.orange)
txt_landauer = Text(text="Dissipação Landauer: 0.0000 pW", position=(-0.82, -0.02), scale=1.0, color=color.white)
txt_qubits = Text(text="Tracked Qubits: 120 | Chaos: 1.0x", position=(-0.82, -0.06), scale=1.0, color=color.white)
...
txt_commands = Text(text="COMMANDS: [UP/DN] Qubits  ||  [S] Multiplica Caos", position=(-0.82, -0.2), scale=0.9, color=color.gold)
txt_ram = Text(text="RAM Buffer Allocation: Active", position=(-0.82, -0.28), scale=0.8, color=color.green)
txt_token = Text(text="Token: Locked", position=(-0.82, -0.31), scale=0.8, color=color.gray)

# --- LOOP DE ATUALIZAÇÃO ---
def update():
    global CONEXAO_ESTAVEL
    
    # Validação agressiva online: Se a thread do heartbeat mudar a flag, mata na hora
    ...
    txt_entropy.text = f"Entropia de Shannon (H): {sphy_sim.shannon_entropy:.4f} bits"
    txt_bohm.text = f"Pressão Quântica (Q): {sphy_sim.bohm_pressure_max:.2f} eV"
    txt_gravity.text = f"Delta G (Δg): {sphy_sim.delta_g:.4f} m/s²"
    txt_landauer.text = f"Dissipação Landauer: {sphy_sim.landauer_dissipation_pw:.4f} pW"
    txt_qubits.text = f"Tracked Qubits: {sphy_sim.num_qubits} | Base Chaos Power: {sphy_sim.escala_ataque_s:.1f}x"
...
def input(key):
    if key in ('up arrow', 'arrow up'):
        if sphy_sim.num_qubits < 1200:
            sphy_sim.num_qubits += 10
            sphy_sim.recalibrar_tensores()
            print(f"[URSINA] Qubits expandidos para: {sphy_sim.num_qubits}")
    elif key in ('down arrow', 'arrow down'):
        if sphy_sim.num_qubits > 20:
            sphy_sim.num_qubits -= 10
            sphy_sim.recalibrar_tensores()
            print(f"[URSINA] Qubits reduzidos para: {sphy_sim.num_qubits}")
    elif key == 's':
        if sphy_sim.escala_ataque_s < 5.0:
            sphy_sim.escala_ataque_s += 0.5

# --- CORDÃO UMBILICAL OBLIGATÓRIO (ONLINE ONLY) ---
def loop_heartbeat():
    global CONEXAO_ESTAVEL
    while CONEXAO_ESTAVEL:
        try:
            # POST com timeout estrito para impedir congelamento de pacotes ou simulações locais offline
            resposta = requests.post(f"{SERVER_URL}/ping?token={SESSÃO_TOKEN}", timeout=2)
...
def autodestruicao_memoria():
    ...
    print("\n💥 [PROTOCOLO DISPARADO] Conexão com Oracle interrompida. Expurgando buffers da RAM...")
    if BUFFER_MEMORIA_VOLATIL is not None:
        del BUFFER_MEMORIA_VOLATIL
    BUFFER_MEMORIA_VOLATIL = None
    CONEXAO_ESTAVEL = False
    print("🔒 Memória higienizada. Pipeline encerrado.")
    sys.exit(0)

def iniciar_ambiente_efemero():
    global SESSÃO_TOKEN, CONEXAO_ESTAVEL, BUFFER_MEMORIA_VOLATIL
    print("🌐 Sincronizando com barramento remoto da Oracle Cloud (161.153.0.202)...")
    try:
        # Tenta estabelecer a ponte HTTP inicial
        reg = requests.get(f"{SERVER_URL}/solicitar_conexao", timeout=3).json()
        SESSÃO_TOKEN = reg["token_acesso"]
        
        # Dispara o switch de segurança persistente
        thread_ping = threading.Thread(target=loop_heartbeat, daemon=True)
        thread_ping.start()
        
        # Puxa os dados binários dinâmicos da nuvem
        ...
        print(f"[SUCCESS] Core validado e alocado na RAM.")
        
        # Só inicializa a janela 3D se as duas etapas HTTP online passarem
        app.run()

    except Exception as e:
        print(f"Falha crítica de infraestrutura online: {e}")
        autodestruicao_memoria()
